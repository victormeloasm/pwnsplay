...
clean_msg() {
  tr '\n' ' ' < "$1" | sed 's/[[:space:]][[:space:]]*/ /g; s/|/ /g' | cut -c1-110
}
...
if command -v clojure >/dev/null 2>&1; then
  run_one Clojure ./fact_clojure.clj "$N"
else
  echo "Clojure|SKIPPED|0|0|clojure command not found" >> "$OUT"
fi
if [[ -x ./bin/fact_fasm ]]; then
  run_one FASM ./bin/fact_fasm "$N"
else
  echo "FASM|SKIPPED|0|0|FASM binary not built" >> "$OUT"
fi
if [[ -x ./bin/fact_cobol ]]; then
  run_one COBOL ./bin/fact_cobol "$N"
else
  echo "COBOL|SKIPPED|0|0|COBOL binary not built" >> "$OUT"
fi
if [[ -x ./bin/fact_pascal ]]; then
  run_one Pascal ./bin/fact_pascal "$N"
else
  echo "Pascal|SKIPPED|0|0|Pascal binary not built" >> "$OUT"
fi
if [[ -x ./bin/fact_basic ]]; then
  run_one BASIC ./bin/fact_basic "$N"
else
  echo "BASIC|SKIPPED|0|0|BASIC binary not built" >> "$OUT"
fi
